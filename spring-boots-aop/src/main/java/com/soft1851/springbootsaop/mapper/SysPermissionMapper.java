...
import com.soft1851.springbootsaop.entity.SysPermission;
...
public interface SysPermissionMapper {
    ...
    @Results({
            ...
            @Result(property = "subList", column = "p_id",
                    many=@Many(select="com.soft1851.springbootsaop.mapper.SysPermissionMapper.getPermissionByParentId")),
    })
    @Select("SELECT * FROM sys_permission WHERE p_id=#{id}")
    ...
    @Results({
            @Result(property = "p_id", column = "p_id"),
            @Result(property = "subList", column = "p_id",
                    one=@One(select="com.soft1851.springbootsaop.mapper.SysPermissionMapper.getPermissionByParentId")),
    })
